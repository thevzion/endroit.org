...
test("the comparison moves explicitly from Agent-centric to Workplace-first", () => {
  ...
  const home = transitionDistrict(initialDistrictState, {
    ...
    perspective: "workplace-first",
  })

  assert.equal(home.perspective, "workplace-first")
...
test("unknown interactions preserve the current comparison", () => {
  const state = { perspective: "workplace-first" }
